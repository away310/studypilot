...
内置 **38 条评测集**（`src/main/resources/eval-set.json`），覆盖 8 篇示例知识库文档，
每道题标注期望命中的文档。运行 `GET /api/eval/run` 或页面右上角按钮即可复现。
以下为 **DashScope text-embedding-v3 实测结果**（2026-09）：

| 模式 | Recall@5 | Top-1 命中 |
|---|---|---|
| 纯向量检索 | 100% | 86.8% |
| 纯关键词检索（BM25） | 100% | 89.5% |
| **混合检索（向量 + BM25）** | **100%** | **92.1%** |

**结论**：混合检索把正确文档排到 top-1 的比例比纯向量高 **+5.3pp**、比纯关键词高 **+2.6pp**，
说明双通道融合在中小知识库上排序更准（召回阶段均已足够，主要差异在排序精度）。

> 该评测集与示例知识库随仓库提供，`mvn spring-boot:run` 后一键可复现；
> 换成你自己的知识库后同样可跑，用于持续验证切分/检索改动是否回退。
